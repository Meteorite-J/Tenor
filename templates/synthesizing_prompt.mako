...
Also keep in mind that prediction markets often have moonshot bias,
i.e. people sometime tend to overweight extreme low-probability outcomes and underweight high-probability ones.
...
or ground it with the actuals from the FRED data (note that the FRED data is usually a month or a quarter behind, so act accordingly).
</task>

<output_format>
Present in a markdown format with sections and sub-sections.
Go from broad (e.g. macro) to narrow (e.g. sector) and finally individual names or indices as top-level sections.
...
This is intended to be consumed daily by a PM as a news memo, so just use the title: Daily Memo - ${today.strftime('%Y-%b-%d')}
Things to avoid:
